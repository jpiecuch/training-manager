...
public class UserWorkoutsController extends AbstractController {
    ...
    public PageResult executions(@PathVariable ApiVersionService.Version version,
                                 ...
                                 @RequestParam(value = "to", required = false) Date to, @RequestParam(value = "maxResults", required = false, defaultValue = "10") Integer maxResults,
        ...
        return versionServices.get(version).retrieveFromRepository(new UserWorkoutCriteria(locale.getLanguage()).addDateRangeRestriction(from, to).setMaxResultsRestriction(maxResults).setOrderBy(orderBy, orderMode, null), Repositories.WORKOUT);
    }
    ...
    public void update(@PathVariable ApiVersionService.Version version, @PathVariable long id, @PathVariable long workoutId, @RequestBody UserWorkoutDto userWorkout) {
